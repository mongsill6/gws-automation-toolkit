...
_blog "INFO" "Drive 증분 백업 시작"
_blog "INFO" "  소스 폴더: ${SOURCE_FOLDER}"
_blog "INFO" "  백업 모드: ${BACKUP_MODE}"
_blog "INFO" "  백업 대상: ${BACKUP_TARGET}"

# ── 마지막 백업 시점 조회 ──
LAST_BACKUP=""
if [ -f "$STATE_FILE" ]; then
  LAST_BACKUP=$(cat "$STATE_FILE")
  _blog "INFO" "  마지막 백업: ${LAST_BACKUP}"
else
  _blog "INFO" "  최초 백업 (전체 백업 수행)"
fi
