...
_b_blog "INFO" "Drive 증분 백업 시작"
_b_blog "INFO" "  소스 폴더: ${SOURCE_FOLDER}"
_b_blog "INFO" "  백업 모드: ${BACKUP_MODE}"
_b_blog "INFO" "  백업 대상: ${BACKUP_TARGET}"

# ── 마지막 백업 시점 조회 ──
LAST_BACKUP=""
if [ -f "$STATE_FILE" ]; then
  LAST_BACKUP=$(cat "$STATE_FILE")
  _b_blog "INFO" "  마지막 백업: ${LAST_BACKUP}"
else
  _b_blog "INFO" "  최초 백업 (전체 백업 수행)"
fi
...
fetch_files() {
  ...
  if [ -n "$LAST_BACKUP" ]; then
    ...
    _b_blog "INFO" "증분 모드: ${LAST_BACKUP} 이후 변경 파일만 조회"
  fi
...
backup_to_local() {
  ...
  if [ -n "$md5" ] && [ -z "$export_mime" ] && command -v md5sum &>/dev/null; then
    ...
    if [ "$local_md5" != "$md5" ]; then
      _blog "WARN" "MD5 불일치: ${file_name} (원본: ${md5}, 로컬: ${local_md5})"
    fi
  fi
  ...
  _blog "OK" "백업 완료: ${file_name} (${size_display})"
  ((BACKED_UP++)) || true
}
...
backup_to_drive() {
  ...
  gws drive files copy --params "$copy_params" >/dev/null 2>&1 || {
    _blog "ERROR" "Drive 복사 실패: ${file_name}"
    ((FAILED++)) || true
    return 0
  }
...
backup_folder_recursive() {
  ...
  while IFS= read -r file; do
    ...
    if [ "$fmime" = "application/vnd.google-apps.folder" ]; then
      _b_blog "INFO" "${indent}📁 하위 폴더 진입: ${fname}"
...
if [ "$BACKUP_MODE" = "local" ]; then
  ...
  _b_blog "INFO" "로컬 백업 경로: ${BACKUP_TARGET}/${TIMESTAMP}"
fi
